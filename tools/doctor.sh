...
if [[ $("$HOME"/.fig/bin/fig app:running) == 1 ]]; then
    ...
    for file in "$HOME"/.profile "$HOME"/.zprofile "$HOME"/.bash_profile "$HOME"/.bashrc "$HOME"/.zshrc; do
        if [[ -f "$file" ]]; then
            ...
            clean_config=$(sed -e 's/#.*$//' "$file" | grep -ve '^$')
            first=$(echo "$clean_config" | sed -n '1p')
            last=$(echo "$clean_config" | sed -n '$p')
    ...
    while IFS= read -ra line; do
        ...
        "Number of specs")
            if ((value > 1)); then
                echo -e "Autocomplete specs installed: $pass"
            else
                echo -e "Autocomplete specs installed: $fail"
                fix "$HOME"/.fig/bin/fig update
            fi
            ;;
        "SSH Integration")
            if [[ $value == true ]]; then
